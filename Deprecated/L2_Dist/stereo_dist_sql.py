...
class ImSplitDecisionLayer(object):
    ...
    def image_diff(self, subim1_size):
        ...
        for k in range(self.subim1_number):
            ...
            self.c.execute("SELECT subim1_ij FROM subim1bank WHERE im_index = %d" % k)
            temp11 = self.c.fetchall()[0][0]
            temp12 = np.fromstring(temp11, dtype=float)
            sub1bank = temp12.reshape(subim1_size, subim1_size, 3)
            self.c.execute("DELETE FROM subim1bank WHERE im_index = %d" % k)
            self.c.execute("SELECT subim2_ij FROM subim2bank WHERE im_index = %d" % k)
            tempfilt = self.c.fetchall()[0][0]
            newfilt = np.fromstring(tempfilt, dtype=float)
            temp = newfilt.reshape(self.subim2_size, self.subim2_size, 3)
            self.c.execute("DELETE FROM subim2bank WHERE im_index = %d" % k)
            ...
            fmappre = 1 / (diff_mat / self.subim1_number)
            # compress to 0 to 1
            for l in range(3):
                fmappre /= np.max(fmappre)

            # Final array of padded feature maps. We pad to get the full range of pixels in decision_maker
            fmap = np.pad(fmappre, ((0, subim1_size), (0, subim1_size), (0, 0)), 'mean')
            # fmappre = fmappre.tostring()
    ...
    def decision_maker(self, subim1_size):
        ...
        for k in range(self.subim1_number):
            ...
            # the length of the side of a square we'll take an integral over
            int_len = 4
            # This code recovers feature maps from the SQL database:
            self.c.execute("SELECT fmap FROM feature_maps WHERE im_index = %d" % k)
        ...
        self.c.execute('CREATE TABLE IF NOT EXISTS offset_map(data TEXT, shape0 INT, shape1 INT)')
        self.c.execute('DELETE FROM offset_map')
        self.c.execute("INSERT INTO offset_map (data, shape0, shape1) VALUES (?,?,?)", (offset_map.tostring(),
            offset_map.shape[0], offset_map.shape[1]))
        self.conn.commit()
        self.c.close()
        self.conn.close()

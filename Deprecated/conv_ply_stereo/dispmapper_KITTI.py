...
lIm, disp = image_resize('../Data/training/colored_0/000000_10.png',
                        '../Data/training/disp_occ/000000_10.png',
                        pic_height_pix)
...
disp += .2*np.ones((disp.shape[0],disp.shape[1]))
disp = disp / np.max(disp)
...
disp = gaussian_filter(disp, sigma=2)
disp = np.float32(disp)
...
h, w = lIm.shape[:2]
f = 5*w                          # guess for focal length
#f = 2*w
Q = np.float32([[1, 0, 0, -0.5*w],
                [0,-1, 0,  0.5*h], # turn points 180 deg around x-axis,
                [0, 0, 0,     -f], # so that y-axis looks up
                [0, 0, 1,      0]])
points = cv2.reprojectImageTo3D(disp[500:1000,:], Q)
colors = cv2.cvtColor(np.float32(lIm[500:1000,:,:]), cv2.COLOR_BGR2RGB)

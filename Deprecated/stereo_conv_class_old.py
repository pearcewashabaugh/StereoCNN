...
class conv_pool_layer(object):
	...
	def __init__(self,im1,im2,pic_dim,filter_size):
		...
		self.filter_bank, self.filter_number, self.filter_bank_avg, \
	...
	def filter_maker(self):
		...
		for i in range(filter_number_h):
			for j in range(filter_number_w):
				filter_bank.append(self.im1[i*self.filter_size:(i+1)
					*self.filter_size,j*self.filter_size:(j+1)*self.filter_size])
				im_bank.append(self.im2[i*self.filter_size:(i+1)
					*self.filter_size,j*self.filter_size:(j+1)*self.filter_size])
		...
		print(im_bank_L2)
		return filter_bank, filter_number, filter_bank_avg,im_bank_L2

	def image_convolver(self):
		#Outputs feature maps
		feature_maps = np.zeros((self.filter_number,self.pic_dim[0],self.pic_dim[1],3))
